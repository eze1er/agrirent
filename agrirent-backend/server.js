...
// ✅ ADD THIS DEBUG LINE
console.log('🔑 Stripe Key Check:', process.env.STRIPE_SECRET_KEY ? '✅ Found (starts with ' + process.env.STRIPE_SECRET_KEY.substring(0, 7) + ')' : '❌ NOT FOUND');
console.log('📁 Current directory:', __dirname);
console.log('📄 Looking for .env in:', require('path').resolve(process.cwd(), '.env'));
...
case 'checkout.session.completed': {
  ...
  const updatedRental = await Rental.findByIdAndUpdate(
    rentalId,
    {
      ...
      'payment.status': 'held_in_escrow', // ✅ Also update payment.status
  ...
  break;
      ...
      case 'payment_intent.succeeded': {
        ...
        console.log(`✅ PaymentIntent réussi (Transaction ID: ${paymentIntent.id})`);
        break;
      }
